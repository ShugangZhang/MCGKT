...
from data_loaders.assist2009 import ASSIST2009  # 如果要训练MCGKT，就用该文件来加载数据，一题多概念
# from data_loaders.assist2009_for_gkt import ASSIST2009  # 如果要训练GKT，就用该文件来加载数据，一题单概念
# from data_loaders.assist2017 import ASSIST2017
from data_loaders.assist2017_for_gkt import ASSIST2017
...
# from models.my_gkt import MYPAM, MYMHA, MYGKT
from models.my_gkt2015 import MYPAM2015, MYMHA2015
...
def main(model_name, dataset_name):
    ...
    elif model_name == "mygkt2015":
        if model_config["method"] == "MYPAM2015":
            # model = PAM(dataset.num_q, **model_config).to(device)
            model = MYPAM2015(dataset.num_q, **model_config).cuda()  # 从这里把邻接矩阵传进去
        elif model_config["method"] == "MYMHA2015":
            # model = MHA(dataset.num_q, **model_config).to(device)
            model = MYMHA2015(dataset.num_q, **model_config).cuda()
    else:
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=batch_size, shuffle=False,  # 改称1
        collate_fn=collate_fn, generator=torch.Generator(device='cuda')
    )
    ...
    """ 
    # 单次模式 
    loss_means, valid_aucs, test_aucs = \
        model.train_model(
            # train_loader, test_loader, num_epochs, opt, ckpt_path
            train_loader, valid_loader, test_loader, num_epochs, opt, ckpt_path, device  # 送入valid而不是test
        )
    """
    ...
    test_aucs_mean = test_aucs.mean()
    test_aucs_std = test_aucs.std()

    test_accs_mean = test_accs.mean()
    test_accs_std = test_accs.std()

    print(f"Test AUC: mean = {test_aucs_mean:.4f}±{test_aucs_std}")
    print(f"Test ACC: mean = {test_accs_mean:.4f}±{test_accs_std}")
    ...
    # 没意义，不想要了
    # with open(os.path.join(ckpt_path, "valid_aucs.pkl"), "wb") as f:
    #     pickle.dump(valid_aucs, f)
    # with open(os.path.join(ckpt_path, "test_aucs.pkl"), "wb") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="The name of the model to train. \
            The possible models are in [dkt, dkt+, dkvmn, sakt, gkt, mcgkt]. \
            The default model is mcgkt."
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
