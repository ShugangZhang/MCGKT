...
class ASSIST2009(Dataset):
    ...
    def preprocess(self):
        ...
        df = df.dropna(subset=["user_id", "problem_id", "order_id", "correct", "skill_name"]).sort_values(by=["order_id"])

        u_list = np.unique(df["user_id"].values)
        q_list = np.unique(df["skill_name"].astype(str).values)
        #------------------------
        ...
        for u in u_list:
            ...
            q_seq = np.array([q2idx[q] for q in df_u["skill_name"]])  # 改版前：skill_name
