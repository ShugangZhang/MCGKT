...
class ASSIST2017(Dataset):
    def __init__(self, seq_len, dataset_dir=DATASET_DIR) -> None:
        ...
        self.dataset_path = os.path.join(
            ...
            self.dataset_dir, "anonymized_full_release_competition_dataset.csv"
        )
        ...
        if seq_len:
            self.q_seqs, self.r_seqs = \
                match_seq_len(self.q_seqs, self.r_seqs, seq_len)  # 本步骤是将seq拆为100长度的若干seqs
    ...
    def preprocess(self):
        ...
        df = df.dropna(subset=["studentId", "problemId", "correct", "skill"])

        u_list = np.unique(df["studentId"].values)
        q_list = np.unique(df["skill"].astype(str).values)
        #------------------------
        ...
        for u in u_list:
            ...
            q_seq = np.array([q2idx[q] for q in df_u["skill"]])
            # q_seq = np.array([q2idx[q] for q in df_u["skill_id"].astype(str)])  # 改版后
...
#     DATASET_DIR = "datasets/ASSIST2017/"
#
#     df = pd.read_csv("../datasets/ASSIST2017/anonymized_full_release_competition_dataset.csv")
#     print(len(df))
#
#     skill_counts = df.groupby('problemId')['skill'].nunique()
#     print(skill_counts)
#
#     multi_skill_ids = skill_counts[skill_counts > 1].index.tolist()
#
#     if multi_skill_ids:
#         for pid in multi_skill_ids:
#             skills = df[df['problemId'] == pid]['skill'].unique()
#             print(f" problemId={pid}: {list(skills)}")

    # conflict_ids = skill_counts[skill_counts > 0]
    # conflicted_rows = df[df['problemId'].isin(conflict_ids)].copy()
    #
    # summary = (
    #     conflicted_rows
    #     .groupby('problemId')['skill']
    #     .unique()
    #     .reset_index()
    #     .rename(columns={'skill': 'conflicting_skills'})
    # )
    #
    # print(summary)