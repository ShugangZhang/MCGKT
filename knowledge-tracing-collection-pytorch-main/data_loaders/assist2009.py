...
from models.utils import match_seq_len, match_seq_len_2009, match_seq_len_2009_corrected
...
class ASSIST2009(Dataset):
    def __init__(self, seq_len, dataset_dir=DATASET_DIR) -> None:
        ...
        self.dataset_path = os.path.join(
            ...
            self.dataset_dir, "skill_builder_data_corrected_collapsed.csv"
        )
    ...
    def generate_prob_graph(self):
        ...
        for u in u_list:  # 遍历所有用户，按照用户来组织条目
            ...
            p_seq = np.array([p2idx[p] for p in df_u["problem_id"]])
    ...
    def generate_pq_matrix(self):
        ...
        p_idx = np.array([p2idx[p] for p in df["problem_id"]])
    ...
    def preprocess(self):
        ...
        df = df.dropna(subset=["user_id", "problem_id", "order_id", "correct", "skill_id"]).sort_values(by=["order_id"])
        ...
        u_list = np.unique(df["user_id"].values)  # 统计所有用户并编号
        ...
        p_list = np.unique(df["problem_id"].values)  # 统计所有问题并编号
        ...
        num_p = p_list.shape[0]
        pq_rel = np.zeros((num_p, num_q), dtype=np.integer)
        df_group = df[["problem_id", "skill_id"]].groupby("problem_id", as_index=False)
        df_group = df_group.agg({'skill_id': 'first'})
        for p, Q in zip(df_group["problem_id"], df_group["skill_id"]):
            for q in Q:
                p_idx = p2idx[p]
                q_idx = q2idx[q]
                pq_rel[p_idx][q_idx] = 1
...
def preprocess():
    # d.preprocess()
    df = pd.read_csv("../datasets/ASSIST2009/test_file.csv")
    print("debug")

    df = df.dropna(subset=["user_id", "problem_id", "order_id", "correct", "skill_id"])   # 283105
    skills = df["skill_id"].astype(str).str.split("_")

    skills = skills.apply(convert_to_int)

    skills = skills.explode().unique()
    ...
    # result = df.groupby('skill_name').size()
    result = df["skill_id"].nunique()

    print(result)


    df1 = df.dropna(subset=["skill_name"]).drop_duplicates(subset=["order_id", "skill_name"])
        # .sort_values(by=["order_id"])

    print(len(df1))

    df2 = df.dropna(subset=["user_id", "problem_id", "order_id", "correct", "skill_name"])
